...
const RESEARCHER_ID = registry.register({
  ...
  provider: 'anthropic',
  system_prompt: 'You are a research specialist. Investigate the topic and save thorough research notes using save_to_memory with key "research_notes".',
  temperature: 0.5,
  max_steps: 3,
  tools: [],
...
const WRITER_ID = registry.register({
  ...
  model: 'claude-sonnet-4-20250514',
  provider: 'anthropic',
  system_prompt: 'You are a writer. Read the research notes from memory and produce a clear, engaging summary. Save it using save_to_memory with key "draft".',
  temperature: 0.7,
  max_steps: 3,
  tools: [],
